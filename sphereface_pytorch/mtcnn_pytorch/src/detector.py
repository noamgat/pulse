...
def detect_faces(image, min_face_size=20.0, thresholds=[0.6, 0.7, 0.8],
    ...
    offsets = output[0].data.detach().cpu().numpy()  # shape [n_boxes, 4]
    probs = output[1].data.detach().cpu().numpy()  # shape [n_boxes, 2]
    ...
    landmarks = output[0].data.detach().cpu().numpy()  # shape [n_boxes, 10]
    offsets = output[1].data.detach().cpu().numpy()  # shape [n_boxes, 4]
    probs = output[2].data.detach().cpu().numpy()  # shape [n_boxes, 2]
...
def run_first_stage(image, net, scale, threshold):
    ...
    output = net(img)
    probs = output[1].data.detach().cpu().numpy()[0, 1, :, :]
    offsets = output[0].data.detach().cpu().numpy()
