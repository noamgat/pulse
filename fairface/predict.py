...
def predidct_age_gender_race(save_prediction_at, imgs_path = 'cropped_faces/'):
    ...
    for index, img_name in enumerate(img_names):
        ...
        debug_dir, debug_img = os.path.split(img_name)
        debug_dir = debug_dir + "_pretrans"
        os.makedirs(debug_dir, exist_ok=True)
        debug_img = os.path.join(debug_dir, debug_img)
        transforms.ToPILImage()(image).save(debug_img)

        image = trans(image)

        debug_dir, debug_img = os.path.split(img_name)
        debug_dir = debug_dir + "_trans"
        os.makedirs(debug_dir, exist_ok=True)
        debug_img = os.path.join(debug_dir, debug_img)
        transforms.ToPILImage()(image).save(debug_img)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv', dest='input_csv', action='store', default='test_imgs.csv',
                        help='csv file of image path where col name for image path is "csv_column_name')
    parser.add_argument('--column_name', dest='csv_column_name', action='store', default='img_path',
                        help='column name for image path')
